...
.PARAMETER WhatIf
    Mode simulation - Affiche ce qui serait fait sans effectuer de modifications réelles.
...
# -----------------------------------------------------------------------------------
# --- FONCTIONS UTILITAIRES PATCH v2.0.2
# -----------------------------------------------------------------------------------
...
function Move-UserProfile {
    ...
    # Vérification de l'espace disque disponible (PATCH UX-002)
    if (-not $WhatIf) {
        Write-StatusMessage "Vérification de l'espace disque disponible..." "Progress"
        $profileSize = Get-FolderSize -Path $selectedProfile.Path
        if (-not (Test-AvailableSpace -SourceSize $profileSize -DestinationDrive $destinationDrive)) {
    ...
    try {
        ...
        # Mode simulation WhatIf (PATCH UX-008)
        if ($WhatIf) {
            $folderList = $foldersToMove.Keys | ForEach-Object { "  • $_" }
            Show-InfoBox -Lines (@(
                "MODE SIMULATION (WhatIf) ACTIVÉ",
                "",
                "Les opérations suivantes SERAIENT effectuées :",
                "",
                "PROFIL À DÉPLACER :",
                "  Utilisateur : $($selectedProfile.Name)",
                "  Source      : $($selectedProfile.Path)",
                ...
                "  1. Création sauvegarde registre",
                "  2. Création dossier destination : $destinationPath",
                "  3. Déplacement de chaque dossier avec robocopy",
                "  4. Mise à jour registre Windows",
                "  5. Notification système",
                "",
                "AUCUNE MODIFICATION N'A ÉTÉ EFFECTUÉE (mode simulation)"
            )) -BorderColor "Yellow"

            Write-StatusMessage "Mode WhatIf : Simulation terminée sans modifications" "Info"
            Write-Host ""
            Read-Host "Appuyez sur Entrée pour revenir au menu principal"
            return
        }
...
function Restore-UserProfile {
    ...
    if ($WhatIf) {
        ...
        $folderList = $foldersToRestore | ForEach-Object { "  • $_" }
        Show-InfoBox -Lines (@(
            "MODE SIMULATION (WhatIf) ACTIVÉ",
            "",
            "Les opérations suivantes SERAIENT effectuées :",
            ...
            "PROFIL À RESTAURER :",
            "  Utilisateur : $($selectedProfile.Name)",
            "  Source      : $($selectedProfile.MovedPath)",
            "  Destination : $($selectedProfile.OriginalPath)",
            "",
            "DOSSIERS À RESTAURER :"
        ) + $folderList + @(
            "",
            "ACTIONS QUI SERAIENT EFFECTUÉES :",
            "  1. Création dossier destination : $($selectedProfile.OriginalPath)",
            "  2. Restauration de chaque dossier avec robocopy",
            "  3. Restauration registre depuis sauvegarde",
...
function Backup-UserProfile {
    ...
    # Mode simulation WhatIf (PATCH UX-008)
    if ($WhatIf) {
        $folderList = $foldersToBackup | ForEach-Object { "  • $_" }
        Show-InfoBox -Lines (@(
            "MODE SIMULATION (WhatIf) ACTIVÉ",
            "",
            "Les opérations suivantes SERAIENT effectuées :",
            "",
            "PROFIL À SAUVEGARDER :",
            "  Utilisateur : $($selectedProfile.Name)",
            "  Source      : $($selectedProfile.Path)",
            "  Destination : $destinationPath",
            "  Taille      : $totalSizeGB GB",
            "",
            "DOSSIERS À SAUVEGARDER :"
        ) + $folderList + @(
            "",
            "ACTIONS QUI SERAIENT EFFECTUÉES :",
            "  1. Création dossier de sauvegarde : $destinationPath",
            "  2. Copie de chaque dossier avec robocopy (mode COPIE, pas de suppression)",
